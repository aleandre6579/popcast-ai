...
vggish_model = TensorflowPredictVGGish(
    graphFilename=MODELS_PATH + "/audioset-vggish-3.pb", output="model/vggish/embeddings"
)
...
happy_model = TensorflowPredict2D(
    graphFilename=MODELS_PATH + "/mood_happy-audioset-vggish-1.pb", output="model/Softmax"
)
party_model = TensorflowPredict2D(
    graphFilename=MODELS_PATH + "/mood_party-audioset-vggish-1.pb", output="model/Softmax"
)
...
def run_essentia_models(audio16k, audio44k):
    ...
    jamendo_dict = {
        jamendo_class: jamendo_value for jamendo_class, jamendo_value in zip(jamendo_classes, jamendo_predictions)
    }
    ...
    jamendo_instrument_dict = {
        ...
        for jamendo_class, jamendo_value in zip(jamendo_instrument_classes, jamendo_instrument_predictions)
    }
...
def process_songs_in_batches(embeddings_filepath, song_paths, lower, upper, batch_size=10):
    with h5py.File(embeddings_filepath, "w") as embeddings_file:
        songs_data_lower, songs_data_upper = [
        ...
        for batch_start in tqdm(
            range(songs_data_lower, songs_data_upper, batch_size),
            desc=f"Processing Batches ({lower}, {upper})",
        ):
...
all_bounds = [(lower, upper) for (lower, upper) in zip(range(28, 48, 4), range(32, 52, 4))]

for bounds in all_bounds:
    lower, upper = bounds
    embeddings_filepath = f"/mnt/f/Alex Stuff/Songs/Embeddings/song_embeddings_{lower}_{upper}.h5"
    songs_data_full = process_songs_in_batches(embeddings_filepath, song_paths, lower, upper, batch_size=125)
